...
public class RoundRobinLab {
		...
		public static void scheduleRoundRobin(List<Process> processes, int timeQuantum) {
			...
			while(queue.size() > 0) {
				...
				int exececuteTime=Math.min(timeQuantum,current.remainingTime);
				currentTime+=exececuteTime;
				
				currentTime = currentTime + exececuteTime; 
				
				current.remainingTime = current.remainingTime - exececuteTime; 
